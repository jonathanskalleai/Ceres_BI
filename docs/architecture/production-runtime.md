...
- O MCP padrao do Supabase e o `project_id` local nao alcancam esse banco
  self-hosted; nao tente diagnosticar producao por eles. A excecao e o proxy
  privado `supabase-self-hosted`, descrito abaixo.
...
### MCP self-hosted no Codex: preflight por sessao local

O projeto tambem tem o MCP opcional `supabase-self-hosted`, configurado em
`.codex/config.toml` para `http://127.0.0.1:18080/mcp`. Esse endereco e local:
ele so funciona enquanto existir um tunel SSH na maquina que esta executando o
Codex. Na VPS, o proxy `supabase-mcp-ssh-proxy` fica deliberadamente privado em
`127.0.0.1:18000`; ele nao deve ser exposto na internet.
...
Depois confira que a porta local esta em escuta:

```bash
lsof -nP -iTCP:18080 -sTCP:LISTEN
```

Se a sessao do Codex ja exibiu `MCP startup incomplete`, abra o tunel e reinicie
o cliente/extensao antes de tentar novamente. Esse erro, isoladamente, significa
que o tunel local esta ausente; nao significa que o Supabase na VPS caiu. O
tunel precisa ser aberto de novo depois de reiniciar a maquina ou quando o
processo SSH encerrar.
