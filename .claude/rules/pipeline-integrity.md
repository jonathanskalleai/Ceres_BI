...
### 7. @scribe e obrigatorio apos @qa PASS (mecanico — `scribe-gate.sh`)

Stop hook: se ha `qa-verdict.json` com PASS fresco (<2h) e NENHUM spawn de
`aivoux-scribe` depois dele, o fechamento da sessao e bloqueado 1x com a
instrucao do PASSO 4.5. Escapes legitimos (unicos):
- SIMPLE sem doc relacionada → criar `.aivoux/gates/docs-na` com o motivo
- @scribe falhou (529) → reportar `DOCS_PENDING` explicitamente ao usuario
...
| `scribe-gate.sh` | Stop | BLOQUEIA fechamento 1x se @qa PASS sem @scribe depois |
...
.aivoux/gates/
  ...
  docs-na                    # escape do scribe-gate (SIMPLE sem doc) — motivo dentro
...
- ❌ Encerrar sessao com @qa PASS e @scribe nunca spawnado (memoria do
  projeto nao registra a entrega; proxima sessao comeca cega)
