...
2. **Profundo, condicional (@security gate):** no modo FULL/auditoria, quando a mudanca toca **superficie
   ...
   ferramentas (grep de segredos, `npm audit`), e emite verdict `SECURE /
...
Alem do gate de construcao, a skill `aivoux-audit-security` usa o `@security` como
...
> **NOTA RUNTIME CODEX:** o Codex nao roda `security-gate.sh`. O gate condicional
> e comportamental — a disciplina de acionar o @security em escopo sensivel e do
> router. `.aivoux/gates/security-verdict.json` serve de registro, nao de
> destravador de hook.
