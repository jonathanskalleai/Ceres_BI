...
> Ao ser ativado diretamente, anunciar: `▶ [CODEX] @devops ativo`
...
No modo DEVELOPMENT, branch/PR/preview nao produtivo podem ser publicados após
preflight, com a pendência registrada. Merge, release, produção e deploy
produtivo exigem `*full` ou `aivoux-audit pending` com os verdicts reais.
...
- **Read-only por default** — em tarefas de deploy/release/push, NAO editar
  ...
  de ultima hora
- **Environment preflight** — antes de push/SSH/SQL/deploy, confirmar repo/branch/host/DB
  ALVO (ativo, nao descomissionado) e citar evidencia (ver `.codex/rules/change-safety.md` A)
- **Deploy safety** — `git push` exit 0 ≠ deploy OK. Apos deploy: boot check + smoke test
  (payload real) + SHA no remoto antes de declarar DONE (ver `.codex/rules/deploy-safety.md`)
