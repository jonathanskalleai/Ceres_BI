#!/usr/bin/env bash
# Runs on the VPS cron, from the checked-out release directory.
# The job token remains in .env and is written only to a mode-600 curl config,
# never to the process list or cron output.
set -euo pipefail
...
echo "==> Narrative team and consultant insights"
curl --fail --silent --show-error --max-time 2700 --config "${CURL_CONFIG}" \
  -X POST "${BASE_URL}/generate-weekly-insights"
echo
