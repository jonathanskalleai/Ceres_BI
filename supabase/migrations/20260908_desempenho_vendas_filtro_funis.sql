-- Migration: Suporte a filtro multiseleção de funil (p_funis) no Dashboard de Desempenho de Vendas
-- Data: 2026-09-08
-- Descrição: Adiciona o parâmetro p_funis (text[]) em rpc_desempenho_vendas_bi_core, rpc_desempenho_vendas_bi,
--            rpc_pedidos_pendentes_esteira, rpc_acoes_pedidos_ganhos e rpc_acoes_negocios_perdidos.

-- 1. Core de Desempenho de Vendas
...
AS $$
    ...
    SELECT
      COALESCE(
        ...
        CASE
          ...
          ELSE make_date(EXTRACT(YEAR FROM CURRENT_DATE)::integer, 1, 1)
        END
      ) AS v_from,
      COALESCE(
        p_to,
      ...
      COALESCE(
        p_ano,
        EXTRACT(YEAR FROM COALESCE(p_from, CURRENT_DATE))::integer
      ) AS v_target_year
  ),
  negocios_canonicos AS (
    ...
    WHERE (
      CASE 
        WHEN p_funis IS NOT NULL AND array_length(p_funis, 1) > 0 
        THEN n.ngo_funil = ANY(p_funis)
        ELSE COALESCE(n.ngo_funil, '') NOT ILIKE '%repasse%'
      END
    )
    ...
    FROM (
      SELECT
        ...
        (
          ...
          FROM mirror.crm_negocios m
          WHERE m.ngo_motivoperda = n.ngo_motivoperda
            AND m.mpp_produtoperdamarca IS NOT NULL
            AND m.mpp_produtoperdamarca != ''
            AND m.mpp_produtoperdamarca != 'Não Informada'
...
-- 2. Wrapper exposto à API Supabase
...
-- 3. Esteira de Pedidos Pendentes com suporte a p_funis
...
pedidos_filtrados AS (
  ...
  WHERE (
    CASE
      WHEN p_funis IS NOT NULL AND array_length(p_funis, 1) > 0
      THEN nc.ngo_funil = ANY(p_funis)
      ELSE COALESCE(nc.ngo_funil, '') <> 'REPASSE DE MAQUINA'
    END
  )
...
-- 4. Pedidos Ganhos Detalhe com suporte a p_funis
...
-- 5. Negócios Perdidos Detalhe com suporte a p_funis
...
GRANT EXECUTE ON FUNCTION public.rpc_pedidos_pendentes_esteira(
  date, date, integer, text, text, text[]
) TO authenticated, service_role, anon;

GRANT EXECUTE ON FUNCTION public.rpc_acoes_pedidos_ganhos(
  date, date, text, text, int, int, text[]
) TO authenticated, service_role, anon;
