-------------------------------------------------------------------------------
-- revoke_public_rpc_acoes_funil_gestao.sql
-- Story: SEC-FIX-1
-- Data: 2026-08-03
--
-- PROPOSITO
--   Revogar EXECUTE de PUBLIC e anon na funcao existente rpc_acoes_funil_gestao.
--   Garantir que apenas authenticated e service_role possam executar.
-------------------------------------------------------------------------------

BEGIN;

REVOKE EXECUTE ON FUNCTION public.rpc_acoes_funil_gestao(date, date, text, text)
  FROM PUBLIC, anon;
...
DO $$
DECLARE
  fn_name text := 'rpc_acoes_funil_gestao';
  row_count int;
BEGIN
