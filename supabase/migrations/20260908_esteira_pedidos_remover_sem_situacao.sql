-- Migration: Correção da Esteira de Pedidos (rpc_pedidos_pendentes_esteira)
-- Data: 2026-09-08
-- Descrição: Remove o agrupamento forçado de pedidos 'Sem Situação' e garante que a esteira
--            reflita estritamente pedidos pendentes reais do CRM ('Aguardando Aprovação' e 'Aguardando Assinatura Cliente').

DROP FUNCTION IF EXISTS public.rpc_pedidos_pendentes_esteira(date, date, integer, text, text);
...
pedidos_dedup AS (
  ...
  WHERE p.pdo_situacaopedido IN ('Aguardando Aprovação', 'Aguardando Assinatura Cliente')
...
aguardando_assinatura AS (
  SELECT
    ...
    COALESCE(SUM(pdo_vlrpedido), 0)::numeric AS valor
  FROM pedidos_filtrados
  WHERE pdo_situacaopedido = 'Aguardando Assinatura Cliente'
),
lista_pedidos AS (
  ...
  FROM (
    SELECT 
      ...
      dth_evento AS "data",
      pdo_situacaopedido AS "situacao",
