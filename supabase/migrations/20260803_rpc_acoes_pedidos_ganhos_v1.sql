...
--   linhas 104-117 (filtered), 125-139 (negocios_base), 145-158 (negocios_canonicos),
--   159-168 (pedidos_dedup).
-------------------------------------------------------------------------------

SAVEPOINT migration_20260803_rpc_acoes_pedidos_ganhos;
...
BEGIN
  RETURN json_build_object(
    'rows', (
      ...
      FROM (
        -- ================================================================
        -- CTE CANONICO: pedidos_dedup (grão = 1 linha por pedido)
        -- Copiado de 20260802_rpc_acoes_funil_gestao_v6_perdidos_negocios.sql:159-168
        -- ================================================================
        ...
        -- ================================================================
        -- CTE FINAL: pedidos do periodo com negocio canonico
        -- ================================================================
        pedidos_periodo AS (
          SELECT
    ...
    'total', (
      -- Total conta pedidos distintos do CTE canonico (sem multiplicacao)
...
COMMENT ON FUNCTION public.rpc_acoes_pedidos_ganhos(date, date, text, text, int, int) IS
  'Drill-down de pedidos ganhos para /bi/acoes v1. Fonte: crm_pedidos Aprovado com negocio canonico Ganho (sem REPASSE). Grão: 1 linha por pedido. Cliente via subquery LIMIT 1. Total conta DISTINCT pdo_codigointerno. GRANT: authenticated+service_role apenas.';

-- ============================================================
-- Padrao de GRANT seguro (ARCH-v3 §3)
-- Postgres SEMPRE da EXECUTE a PUBLIC por padrao.
-- GRANT TO sem REVOKE FROM NAO remove o acesso anonimo.
-- ============================================================
...
ROLLBACK TO SAVEPOINT migration_20260803_rpc_acoes_pedidos_ganhos;
RELEASE SAVEPOINT migration_20260803_rpc_acoes_pedidos_ganhos;
