-------------------------------------------------------------------------------
-- rpc_acoes_bi v4  (supersede 20260723_rpc_acoes_bi_v3.sql)
--
-- MUDANCAS EM RELACAO A v3
--
-- 1) REMOVE a chave `tabelaAcoes` (e a CTE tabela_acoes). A tabela de detalhe
--    virou RPC propria: `public.rpc_acoes_detalhe` (20260724_rpc_acoes_detalhe.sql).
--    Motivo: rpc_acoes_bi roda 4x no app (2x em AcoesSection para o comparativo
--    de trend + 2x em usePainelKPIsRpc, que le SOMENTE `kpis`). Carregar ~660
--    linhas com observacao de ~208 chars no payload monolitico faria esse peso
--    viajar 4 vezes, 2 delas para uma tela que nem renderiza tabela.
--
-- 2) SUBSTITUI o KPI unico `valorNegociado` por SEIS campos por status:
--    valorAberto/negociosAberto, valorGanho/negociosGanho, valorPerdido/negociosPerdido.
--
-- 3) CIDADE padronizada em `cli_cidade` (cidade do CLIENTE, via
--    mirror.crm_carteira_clientes) no KPI `cidades` E no filtro `p_cidade`.
--    Ate a v3 o KPI e o filtro usavam `emp_cidade` (cidade da FILIAL = 4 valores)
--    enquanto os graficos ja usavam `cli_cidade` (90 valores): o usuario filtrava
--    uma coisa e lia outra.
--
-- 4) CORRIGE FAN-OUT da carteira em TODAS as CTEs que a usam (por_cidade,
--    por_vendedor_cidade, clientes_atendidos). A PK de crm_carteira_clientes e
--    (cli_idcliente, usr_idusuario): o mesmo cliente aparece uma vez POR VENDEDOR,
--    entao `LEFT JOIN ... ON cli_idcliente = cli_idcliente` MULTIPLICA as acoes.
--    Medido no banco vivo (julho/2026): 664 acoes viravam 1.472 linhas (2,22x) e o
--    ranking de cidades saia ERRADO, nao so inflado (Faxinal dos Guedes aparecia em
--    1o com 114 acoes; com a correcao cai fora do top 5 e Mafra assume com 61).
--    Correcao: LEFT JOIN LATERAL (... LIMIT 1) ON TRUE — 1 linha de carteira por acao.
--    Verificado que cli_cidade NAO e ambiguo por cliente (0 clientes com >1 cidade
--    distinta na carteira), entao o LIMIT 1 e deterministico na pratica.
-------------------------------------------------------------------------------
...
BEGIN
  ...
  -- (Em Andamento / Ganho / Perdido — verificado: nao ha 4o valor nem NULL).
  -- `outros_*` existe de proposito: se o ERP introduzir um 4o status, ele NAO
  -- some em silencio — a soma dos 3 cards deixa de fechar com negociosTocados e
  -- o @qa detecta. Falha por omissao e a pior falha possivel em BI.
  valores_status AS (
    SELECT
