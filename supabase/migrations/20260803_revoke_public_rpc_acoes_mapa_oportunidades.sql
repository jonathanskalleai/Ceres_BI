-------------------------------------------------------------------------------
-- revoke_public_rpc_acoes_mapa_oportunidades.sql
-- Story: SEC-FIX-1
-- Data: 2026-08-03
--
-- PROPOSITO
--   Revogar EXECUTE de PUBLIC e anon na funcao existente rpc_acoes_mapa_oportunidades.
--   Garantir que apenas authenticated e service_role possam executar.
-------------------------------------------------------------------------------

BEGIN;

REVOKE EXECUTE ON FUNCTION public.rpc_acoes_mapa_oportunidades(text, text)
  FROM PUBLIC, anon;
...
DO $$
DECLARE
  fn_name text := 'rpc_acoes_mapa_oportunidades';
  row_count int;
BEGIN
