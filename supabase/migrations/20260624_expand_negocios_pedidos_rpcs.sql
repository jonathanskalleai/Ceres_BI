-- Migration: Expand rpc_negocios_bi and rpc_pedidos_bi with additional aggregations
-- Adds: velocidadeFunil + duracaoMediaTotal to negocios (from mirror.crm_funil_etapa)
-- Adds: porGrupoProduto + porMarcaProduto to pedidos (from mirror.crm_pedidos_item)
-- Autor: @dev (Dex)
-- Data: 2026-06-24
-- Eliminates: useFunilData, usePedidosItensData (client-side aggregation)

-------------------------------------------------------------------------------
-- RPC 1: rpc_negocios_bi (REPLACE — adds velocidadeFunil + duracaoMediaTotal)
-------------------------------------------------------------------------------
...
BEGIN
  ...
  -- Velocidade do funil — dias medios por etapa (from crm_funil_etapa)
  velocidade AS (
  ...
  -- Duracao media total (all stages combined)
  duracao_total AS (
...
-------------------------------------------------------------------------------
-- RPC 2: rpc_pedidos_bi (REPLACE — adds porGrupoProduto + porMarcaProduto)
-------------------------------------------------------------------------------
