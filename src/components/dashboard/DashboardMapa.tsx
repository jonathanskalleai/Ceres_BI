...
export const DashboardMapa = ({ data, filters }: Props) => {
  ...
  const center: [number, number] = regions.length > 0
    ? [regions.reduce((s, r) => s + r.lat, 0) / regions.length, regions.reduce((s, r) => s + r.lng, 0) / regions.length]
    : [-26.5, -52.0];
    ...
    <div className="p-6 space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground">
          Localização geográfica das ações por região — {regions.length} regiões, {registroPoints.length} registros
        </p>
      </div>
