...
function matches(row: NegocioPerdidoRow, term: string): boolean {
  return [row.cliente, row.cidade, row.consultor, row.negocioNumero, row.produto, row.observacaoNegocio]
    ...
    >
      <div className="overflow-x-auto max-h-[480px] overflow-y-auto">
        <table className="w-full text-xs border-collapse">
          ...
          <tbody>
            {filtered.map((row, i) => (
              <tr key={`${row.negocioNumero}-${i}`} className="border-t border-[var(--voux-card-border)]/30 hover:bg-foreground/[0.02] transition-colors">
                <td className={`${TD} tabular-nums font-mono whitespace-nowrap align-top`}>#{row.negocioNumero}</td>
                ...
                <td className={`${TD} whitespace-nowrap tabular-nums text-right align-top`}>{formatDateTimeBR(row.dataFechamento)}</td>
                <td className={`${TD} text-right tabular-nums whitespace-nowrap font-bold text-red-600 dark:text-red-400 align-top`}>{brl(row.valorPerdido)}</td>
              </tr>
            ))}
            {filtered.length === 0 && (
              <tr>
